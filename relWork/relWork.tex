...
Semantics are given, much like the normal PN, with similar interpretations as
...
\begin{center}
  ...
  \end{center}
% semantics  
Reaction (or rules) have the usual qualitative interpetation: anything that
'matches` the lhs in the state can be replaced by the rhs. Kappa
also has a stochastic intepretation as a CTMC where the states are
...
% examples
Using this site-graph approach we can represent the 1-D arangement of cells from
our example:
\begin{center}
...
\todo{figure of rules as CPN transitions}
...
Grammars of \citet{mjolsness2006stochastic}.
