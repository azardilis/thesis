In this chapter I will introduce Chromar (formally introduced in the next
...
In systems theory, as was discussed in Chapter~\ref{chp:intro},
there is a stress on the organisation side of living things. In this view there
is the assumption then that we can identify unities, that is objects with
well-defined boundaries that form this organisation through their relations
\citep[see for example Section 9 in;][]{varela_autopoiesis:_1974}. In Chromar
discrete entities, agents, are the main entities in the language so writing
systems models of many kinds should be quite natural.

Starting from Biology, developmental biology is a field where the systems view
was recognised perhaps because there you have cells, which are the most
recognised discrete unities in the biological world. Therefore developmental
models should feel quite natural in Chromar. In development there are usually
two levels, the molecular level creating a field over some domain and the tissue
level that grows on top of that domain. We have seen a simple example of this
with the root example above but one could easily imagine 'recipes' for writing
the most common dynamics at both the molecular and tissue levels. For example,
starting with a $\mathrm{Cell(c : \mathrm{Real})}$ agent for a cell with some
concentration of chemical $c$, diffusion along with other molecular rules
(production, degradation, reactions) can be easily written. At the tissue level,
cell division should also be possible with some mechanism of defining the
neighbour relation. Beyond that when going into tissue mechanics that give rise
to shape \citep[morphodynamics, see for example for
plants;][]{formosa-jordan2018} then we could have more agent types to represent
the walls between the cells and even specific mechanisms for doing that like
agents for imaginary springs between cells (a common abstrction for the
forces. The computational requirements are then higher, which might make the
current Chromar implementation not suitable, but the description of the model in
Chromar even without simulation should still be very useful as a specification
of the understanding about a system.
...
models in Chapter~\ref{chp:fms}. Population with structure of some kind (\eg
spatial) can also be encoded given a suitable representation of relations (as we
did above for an array of cells for example). Other quantities (\ie not discrete
objects -- agents) can be mixed as well by encoding as agent attributes or
fluents. If we consider population models as a special case of evolutionary
models \citep{doebeli_towards_2017} then these could be encoded as well.  Again
computationally with, for example, performing large population simulations at
the individual level might not be preferred, but a description of the rules in
Chromar could still be valuable as documentation of our understanding.

Beyond biology, agent-based modelling where descriptions are made, again, with
discrete entities as the main unit is used in other disciplines
\citep{abar2017agent}. The main characteristics is the state of the agents and
their interactions \citep{macal2010tutorial}. Both of these should be natural in
Chromar with agent attributes for the dynamic agent state and rules for their
...
(transitions) \\
  ...
  \end{tabular}}
\end{center}
    \caption{A comparison of indicative languages across the categories we
    identified in the previous chapter (Chapter~\ref{chp:relWork}). Comparison
    is betwen Petri Nets (abbr. PN), Coloured Petri Nets (CPNs), Dynamcical
    Grammars (abbr. DG), Simile, and Chromar.}
      \label{tab:comp}
\end{table}
...
My embedding in Haskell lifts some constraints of modelling languages and, I
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/systemsBio/flow.eps}
    \caption{The workflow in systems biology and the.}
    \label{fig:flowSystems}
\end{figure}

The importance of the choice of language becomes even more apparent in this
...
specific requirements in the modelling process. However since a Chromar model
has a CTMC intepretation, static and dynamic analysis methods on CTMCs should
also be applicable for Chromar models. We define, for example, the simulation of
a Chromar model using the Stochastic Simulation algorithm (SSA) for drawing
trajectories from a CTMC (see Chapter~\ref{chp:chromarForm}). Other static
analyses of CTMCs are also available. Probabilistic (or stochastic) model
checking, for example, is a particularly powerful one
...
\citep{Barnat_Brim_2012}. % Formulas could define desired behaviour of a particular system (\eg
% it oscillates or it reaches a particular state), which means that the obtained
% solutions automatically possess some properties of interest.

The only concrete realisation of Chromar is through the Haskell embedding (see
...
restricted to finite state-spaces \citep[see for example PRISM
tool;][]{Kwiatkowska_Norman_Parker_2011}.

\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{figures/systemsBio/haskChrom.eps}
    \caption{}
    \label{fig:haskChrom}
\end{figure}
...
The existence of fluents further complicates the relation to formal framework of
...
Finally, it should be possible to develop language specific techniques similar
...
means that gives greater flexibility in realisations of Chromar.
